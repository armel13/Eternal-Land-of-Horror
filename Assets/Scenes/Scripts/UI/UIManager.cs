...
public class UIManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        StartCoroutine(OnDelayUpdateUI());
    ...
    void UpdateCommonKey()
    {
        ...
        {
            if (playersInventory.commonKeys > 1) commonKeysText.SetText("0x");
            else commonKeysText.text =  playersInventory.commonKeys.ToString() + "x";  // Memperbarui teks jumlah kunci biasa
        }
     ...
     void UpdateUncommonKey()
    {
        ...
        {
            if (playersInventory.uncommonKeys > 1) commonKeysText.SetText("0x");
            else uncommonKeysText.text =  playersInventory.uncommonKeys.ToString() + "x";  // Memperbarui teks jumlah kunci tidak biasa
        }
    ...
    public void Retry()
    {
        ...
        InfoManager.Instance.LoadStats(); //Using saved data as retry
        //InfoManager.Instance.NewGameStats(true); //New data after retry

        Time.timeScale = 1;  // Mengatur waktu dalam permainan kembali ke kecepatan normal

        if (string.Equals("House1", SceneManager.GetActiveScene().name) || string.Equals("House2", SceneManager.GetActiveScene().name) ||
            string.Equals("House3", SceneManager.GetActiveScene().name) || string.Equals("House4", SceneManager.GetActiveScene().name) ||
            string.Equals("Island", SceneManager.GetActiveScene().name))
        {
            //SaveManager.GetInstance().UpdateSavedDuration(Mathf.RoundToInt(900));
            SceneManager.LoadScene(2);

            if (InfoManager.Instance.currentCheckpointIndex == 0)
            {
        ...
        if (string.Equals("Forest", SceneManager.GetActiveScene().name))
        {
            //SaveManager.GetInstance().UpdateSavedDuration(Mathf.RoundToInt(360));
            SceneManager.LoadScene(1);
            return;
        }

        if (string.Equals("BossCave", SceneManager.GetActiveScene().name))
        {
