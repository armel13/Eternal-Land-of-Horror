...
public class Player : MonoBehaviour, IDamageable
{
    ...
    protected virtual void Start()
    {
        ...
        if (string.Equals(SceneManager.GetActiveScene().name, "Forest") || string.Equals(SceneManager.GetActiveScene().name, "Island"))
        {
            //if (InfoManager.Instance.NewPlayerPosition != Vector2.zero)
            //{
            //    StartCoroutine(OnDelayMove());
            //}
            StartCoroutine(OnDelayMove());
        }
        else
        {
            if (MissionDisplay.GetInstance() != null) MissionDisplay.GetInstance().SetMissionIndex(InfoManager.Instance.currentMissionIndex);
        }
        ...
        {
            StartCoroutine(DelayInitializeFunction());
            //if (!hasSaveData && (OpeningLevel1Dialogue.GetInstance() != null) && !InfoManager.Instance.hasSeenOpeningDialogue) OpeningLevel1Dialogue.GetInstance().StartLevel1Dialogue();
            //else if (Timer.GetInstance() != null) Timer.GetInstance().StartTimer();
        }
    }

    IEnumerator DelayInitializeFunction()
    {
    ...
    IEnumerator OnDelayMove()
    {
        yield return new WaitForSeconds(0.1f);
        if(InfoManager.Instance.isFromHouseInterior)
        {
            InfoManager.Instance.isFromHouseInterior = false;
            transform.position = new Vector2(7.5f,-11f);
        }
        else if (InfoManager.Instance.isFromHouseHouse1)
        {
            InfoManager.Instance.isFromHouseHouse1 = false;
            transform.position = new Vector2(48f, -17f);
        }
        else if (InfoManager.Instance.isFromHouseHouse2)
        {
            InfoManager.Instance.isFromHouseHouse2 = false;
            transform.position = new Vector2(26.5F, -26F);
        }
        else if (InfoManager.Instance.isFromHouseHouse3)
        {
            InfoManager.Instance.isFromHouseHouse3 = false;
            transform.position = new Vector2(35.4f, 2.4f);
        }
        else if (InfoManager.Instance.isFromHouseHouse4)
        {
            InfoManager.Instance.isFromHouseHouse4 = false;
            transform.position = new Vector2(75.4f, -28.2f);
        }
        else
        {
            if (CheckpointList.GetInstance() != null) InfoManager.Instance.NewPlayerPosition = CheckpointList.GetInstance().CheckpointListTransform[InfoManager.Instance.currentCheckpointIndex].position;
            transform.position = InfoManager.Instance.NewPlayerPosition;
        }
        if(MissionDisplay.GetInstance() != null) MissionDisplay.GetInstance().SetMissionIndex(InfoManager.Instance.currentMissionIndex);
        //if (string.Equals(SceneManager.GetActiveScene().name, "Island"))
        //{
        //    transform.position = InfoManager.Instance.NewPlayerPosition;
        //}
    }
    ...
    public void TakeDamage(float damageTaken, GameObject damageGiver)
    {
        ...
        SoundsManager.GetInstance().PlayClip(SoundsManager.Sound.PlayerDamaged);
   ...
   public void SetHasSaveDataValue(bool value)
    {
        hasSaveData = value;
    }
